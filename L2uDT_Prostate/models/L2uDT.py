...
class refine_block(nn.Module):
    def __init__(self, c, k):
        ...
        self.conv1 = nn.Conv3d(c, k, 1)#, bias=False)(num_in, num_out, kernel_size=kernel_size, padding=padding,stride=stride, groups=g, bias=False)
    ...
    def forward(self, x):
        ...
        ref_x = refine_feat.view(b, c, d, h, w)                                # b * c * h * w
...
class MFunit(nn.Module):
    def __init__(self, num_in, num_out, g=1, stride=1, d=(1,1),norm=None):
        ...
        if num_in != num_out or stride != 1:
            ...
            if stride == 2:
                self.conv2x2x2_shortcut = Conv3d_Block(num_in, num_out, kernel_size=2, stride=stride, padding=0, norm=norm)
            if stride == (2,2,1):
                self.conv2x2x2_shortcut = Conv3d_Block(num_in, num_out, kernel_size=stride, stride=stride, padding=0, norm=norm)
...
class MFNet(nn.Module): #
    ...
    def __init__(self, c=2,n=32,channels=128,groups = 16,norm='bn', num_classes=4, num_classes_1=4, num_classes_2=4):
        ...
        self.encoder_block3 = nn.Sequential(
            MFunit(channels, channels*2, g=groups, stride=(2,2,1), norm=norm), # H//8
            MFunit(channels * 2, channels * 2, g=groups, stride=1, norm=norm),
            MFunit(channels * 2, channels * 2, g=groups, stride=1, norm=norm)
        )

        self.encoder_block4 = nn.Sequential(# H//8,channels*4
            MFunit(channels*2, channels*3, g=groups, stride=(2,2,1), norm=norm), # H//16
            MFunit(channels*3, channels*3, g=groups, stride=1, norm=norm),
            MFunit(channels*3, channels*2, g=groups, stride=1, norm=norm),
        )

        self.upsample1 = nn.Upsample(scale_factor=(2,2,1), mode='trilinear', align_corners=False) # H//8
        self.decoder_block1 = MFunit(channels*2+channels*2, channels*2, g=groups, stride=1, norm=norm)

        self.upsample2 = nn.Upsample(scale_factor=(2,2,1), mode='trilinear', align_corners=False) # H//4
        self.decoder_block2 = MFunit(channels*2 + channels, channels*8, g=groups, stride=1, norm=norm)

        self.upsample3 = nn.Upsample(scale_factor=2, mode='trilinear', align_corners=False) # H//2
        self.decoder_block3 = MFunit(channels*8 + n, n, g=groups, stride=1, norm=norm)
        ...
        self.RM0 = refine_block(channels*8, 48)
        self.RM1 = refine_block(channels*8, 96)
    ...
    def forward(self, x, feature=None, phase='original'):
        if phase == 'original':
            # Encoder
        ...
        #    print('x4:', x4.shape)
            ...
            result0, cluster0 = self.RM0(y2)
            result1, cluster1 = self.RM1(y2)
            # result2, cluster2 = self.RM2(feature)
            y2 = (result0+result1)/2.
        else:
            x1 = self.encoder_block1(x)# H//2 down
            y2 = feature
            result0, cluster0 = self.RM0(y2)
    ...
    #    y4 = self.softmax(y4)
        return y2, [cluster0, cluster1], y4


class L2uDT(MFNet): # softmax
    # [128]  Flops:  27.045G  &  Params: 3.88M
    def __init__(self, c=2,n=32,channels=128, groups=16,norm='bn', num_classes=3, num_classes_1=3, num_classes_2=3):
        super(L2uDT, self).__init__(c,n,channels,groups, norm, num_classes, num_classes_1, num_classes_2)
        ...
        self.encoder_block3 = nn.Sequential(
            DMFUnit(channels, channels*2, g=groups, stride=(2,2,1), norm=norm,dilation=[1,2,3]), # H//8
...
if __name__ == '__main__':
    ...
    x = torch.rand((1,2,256,256,32),device=device) # [bsize,channels,Height,Width,Depth]
    model = DMFNet(c=2, groups=16, norm='sync_bn', num_classes=3)
    model.cuda(device)
    y = model(x)
    print(y.shape)
