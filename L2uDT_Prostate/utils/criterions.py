...
__all__ = ['dice_loss','jac_loss','GeneralizedDiceLoss','FocalLoss']
...
def dice(output, target,eps =1e-5): # soft dice loss
    ...
    num = 2*torch.sum(output * target, dim=(1,2,3))
    den = torch.sum(output, dim=(1,2,3)) + torch.sum(target, dim=(1,2,3)) + eps
...
def dice_loss(output, target,alpha=1e-5):
    # output: [-1,3,H,W,T]
...
#    logging.info('1:{:.4f} | 2:{:.4f}'.format(1-loss1.data, 1-loss2.data))
    return loss1+loss2

def jac_loss(output, target,eps=1e-5): #
...
#    logging.info('1:{:.4f} | 2:{:.4f}'.format(loss1.data, loss2.data))
