...
class CVCDataset(Dataset):
    def __init__(self, dataset_name, cfg, period, aug=False):
        ...
        self.root_dir = os.path.join(cfg.ROOT_DIR,'../MetaSeg/data')
        ...
        self.ann_dir = os.path.join(self.dataset_dir, 'GT')
        self.seg_dir = os.path.join(self.dataset_dir, 'GT')
        ...
        if 'train' in self.period or 'valid' in self.period:
    ...
    def save_result_train(self, result_list, model_id):
        ...
        """
        i = 1
        folder_path = os.path.join(self.rst_dir,'%s_%s_test_RM_cls'%(model_id,self.period))
        if not os.path.exists(folder_path):
            os.makedirs(folder_path)
        for sample in result_list:
            file_path = os.path.join(folder_path, '%s'%sample['name'])
            ...
            img[:,input_img.shape[1]*1:input_img.shape[1]*2, :] = pred_img
            img[:,input_img.shape[1]*2:, :] = lab_img
            cv2.imwrite(file_path, img)
            # print('[%d/%d] %s saved'%(i,len(result_list),file_path))
            i+=1

    def save_result_train_2RM(self, result_list, model_id):
        """Save test results
        ...
        for sample in result_list:
            ...
            input_img = cv2.imread(img_file)
            input_img = cv2.resize(input_img, (sample['predict'].shape[0],sample['predict'].shape[1]))
            pred_seg_img1 = np.stack([sample['predict_seg1'], sample['predict_seg1'], sample['predict_seg1']]).transpose((1,2,0))
            pred_seg_img2 = np.stack([sample['predict_seg2'], sample['predict_seg2'], sample['predict_seg2']]).transpose((1,2,0))
            pred_img = np.stack([sample['predict'], sample['predict'], sample['predict']]).transpose((1,2,0))
            lab_img = np.stack([sample['label'], sample['label'], sample['label']]).transpose((1,2,0))

            img=np.zeros([input_img.shape[0], input_img.shape[1]*5, input_img.shape[2]])
            img[:,:input_img.shape[1], :] = input_img
            img[:,input_img.shape[1]:input_img.shape[1]*2, :] = pred_seg_img1
            img[:,input_img.shape[1]*2:input_img.shape[1]*3, :] = pred_seg_img2
    ...
    def save_meta_result_train(self, result_list, model_id):
        folder_path = os.path.join(self.rst_dir,'%s_%s_mixup_cls'%(model_id,self.period))
        if not os.path.exists(folder_path):
            os.makedirs(folder_path)
        for sample in result_list:
            file_path = os.path.join(folder_path, '%s'%sample['name'])
            input_img = np.stack([sample['input'][2,:,:], sample['input'][1,:,:], sample['input'][0,:,:]]).transpose((1,2,0))
            pred_img = np.stack([sample['predict'], sample['predict'], sample['predict']]).transpose((1,2,0))
            lab_img = np.stack([sample['label'], sample['label'], sample['label']]).transpose((1,2,0))
            metalab_img = np.stack([sample['metalabel'], sample['metalabel'], sample['metalabel']]).transpose((1,2,0))

            img=np.zeros([input_img.shape[1], input_img.shape[1]*4, 3])
            img[:,:input_img.shape[1], :] = input_img
            img[:,input_img.shape[1]:input_img.shape[1]*2, :] = pred_img
            img[:,input_img.shape[1]*2:input_img.shape[1]*3, :] = lab_img
            img[:,input_img.shape[1]*3:, :] = metalab_img
            cv2.imwrite(file_path, img)
    ...
    def save_result_train_weight(self, result_list, model_id):
        ...
        for sample in result_list:
            ...
            pred_img = np.stack([sample['predict'], sample['predict'], sample['predict']]).transpose((1,2,0))
            lab_img = np.stack([sample['label'], sample['label'], sample['label']]).transpose((1,2,0))
            v_class = np.stack([sample['v_class'], sample['v_class'], sample['v_class']]).transpose((1,2,0))
            ...
            img[:,input_img.shape[1]:input_img.shape[1]*2, :] = pred_img
            img[:,input_img.shape[1]*2:input_img.shape[1]*3, :] = lab_img
            img[:,input_img.shape[1]*3:input_img.shape[1]*4, :] = v_class
            cv2.imwrite(file_path, img)
            # print('[%d/%d] %s saved'%(i,len(result_list),file_path))
